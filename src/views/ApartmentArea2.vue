...
import imgUrl from "../../public/static/images/video.png";
...
export default {
  ...
  methods: {
    ...
    async AsyncInitModel() {
      ...
      this.initOtherModel();
      this.render();
      this.groupIndex = this.scene.children.findIndex(
          (_) => _.type === "Group"
      );
    },
    // 初始化场景
    ...
    initModelMed(url) {
      ...
      return new Promise((resolve, reject) => {
        // this.axes = new THREE.AxesHelper(6000);
    ...
    initComposerMed() {
      ...
      // 添加到renderPass通道，这个通道会渲染场景，但不会将结果输出到屏幕上
      const renderPass = new RenderPass(this.scene, this.camera);
      // 先添加renderPass
      this.effectComposer.addPass(renderPass);
      // 再添加effectComposer，渲染到屏幕
    ...
    updateLoop(num) {
      ...
      // 动态更新纹理偏移 播放关键帧动画 产生火焰然后效果
      this.texture.offset.x = Math.floor(this.t) / num;
    },
    render() {
      ...
      // effectComposer 里面是对renderer渲染器进行后期处理，则可以在他身上进行renderer()
      if (this.click) {
    ...
    createFlame(size, x, y, z) {
      ...
      this.texture = textureLoader.setPath("/static/images/")
          .load("fire.png");
      // 设置火焰图的帧数
      const num = 15;
      // .repeat 方法设置uv两个方向
      // 1/num: 从图像上截图一帧火焰
      this.texture.repeat.set(1 / num, 1);
    ...
    createScanEffect() {
      ...
      const scan_material = new THREE.MeshBasicMaterial({
        color: "orange",
        map: new THREE.TextureLoader().setPath("/static/images/")
            .load("scan.png"),
        side: THREE.DoubleSide, // 两面可见
        transparent: true, // 需要开启透明度计算，否则着色器透明度设置
        depthTest: false,
      })
